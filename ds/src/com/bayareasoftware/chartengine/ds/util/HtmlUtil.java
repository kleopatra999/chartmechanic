...
public final class HtmlUtil {
    ...
    /* need to recursively get all the children of a given type.... */
    ...
    public static String getText(Node elementNode, int maxLen) {
        ...
        for (Node node : getChildren(elementNode)) {
            ...
            case Q:
                ...
                break;
            case DEL: case STRIKE:
                continue;
            case TT: case I: case B: case BIG: case SMALL: case CENTER:
            case EM: case STRONG: case DFN: case CODE: case SAMP: case KBD:
            case VAR: case CITE: case INS: case P: case U: case UL: case S:
            case SUB: case SUP: case FONT: case A: case SPAN: case ABBR:
                appendText(sb, node);
                break;
            case BR:
                sb.append('\n');
                ...
                break;
            default:
                //System.err.println("*HtmlUtil: unknown tag in getText(): " + node.getNodeName());
                break;
            }
            if (sb.length() >= maxLen) {
                break myloop;
    ...
    private static StringBuilder appendText(StringBuilder sb, Node elementNode) {
        for (Node node : getChildren(elementNode)) {
            if (node.getNodeType() == Node.TEXT_NODE) {
                ...
                case EM: case STRONG: case DFN: case CODE: case SAMP: case KBD:
                case VAR: case CITE: case INS: case P: case U: case UL: case S:
                case SUB: case SUP: case FONT: case A: case SPAN:
                    appendText(sb, node);
                    break;
                case BR:
                    sb.append('\n');
    ...
    public static List<TableSynopsis> tables2synopses(List<HtmlTable> tables)   
    {
        ...
        for (HtmlTable t : tables) {
            if (t.getTotalRows() < 2) {
                // ignore tables with only a single row
                count++;
                continue;
            }
            //int totalRows = t.getTotalRows();
            List<String[]> data = t.getData();
            
            int maxCols = 0; // max # of columns in the data
            ...
            if (allNulls || maxCols == 1) {
                // skip tables that only have one column or have all empty values
                count++;
                continue;
            } 
            ...
            if (tid != null) {
                ...
                // if the table id is missing, use the table index
    ...
    public static List<HtmlTable> getTableSynopses(Document doc) {
        ...
        //Node table = getTable(tableNodes, hi.getTableId());
        List<HtmlTable> ret = new ArrayList<HtmlTable>();
        for (Node n : tableNodes) {
            HtmlTable ht = new HtmlTable(n,SYNOPSIS_ROWS);
            // add to list of HtmlTable even if the table has 'empty' data.
            // will get sorted out in tables2synopses
            ret.add(ht);
        }
        return ret;
    }
